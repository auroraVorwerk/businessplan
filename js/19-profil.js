...
async function renderProfil(){
  ...
  box.innerHTML = `
    <div class="asec">
      <h3>Profil</h3>
      <p class="sub">${rolleName(me.rolle)} · letzte Anmeldung ${letzter}</p>
      <div class="grp">
...
async function renderDelete(){
  ...
  try{
    ...
    inaktiv = alle.filter(u => u.dk !== me.dk && istKBSatz(u)
  ...
  box.innerHTML = `
    ...
    ${istAdmin() ? `<div class="asec">
      <h3>Rolle ändern</h3>
      <p class="sub">Die DK-Nummer bleibt immer gleich. Wer aufsteigt oder absteigt,
        bekommt hier nur eine andere Rolle – Daten, Anmeldung und Passwort bleiben unberührt.</p>
      <div class="grp">
        ${fld("rolDk","DK-Nummer","","text",'autocomplete="off" spellcheck="false"')}
        <div class="field"><label>Neue Rolle</label>
          <div class="seg" id="rolSeg">
            <button type="button" data-rolle="TL" aria-pressed="true">Teamleiter</button>
            <button type="button" data-rolle="KB" aria-pressed="false">Kundenberater</button>
          </div></div>
      </div>
  ...
  if(rolGo) rolGo.onclick = async ()=>{
    ...
    err.hidden = true;
    ok.textContent = `${u.vorname} ${u.nachname} ist jetzt ${rolleName(rolle)}.`;
    ok.hidden = false;
  };
...
async function loeschenStarten(dk, vor, nach){
  ...
  if(!istKBSatz(u) && !istAdmin()) return zeig("Nur Kundenberater lassen sich hier löschen.");
