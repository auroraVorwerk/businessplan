...
/* ---------- Rollen ----------
   Die DK-Nummer sagt nichts mehr über die Rolle aus. Massgeblich ist allein
   das Feld "rolle" im Nutzerdatensatz: "TL", "KB" oder "Admin".
   Alte Schreibweisen ("Teamleader", "Kundenberater") und das alte Kuerzel
   TL-DK werden weiterhin verstanden, damit nichts umkippt, solange noch
   nicht alle Datensaetze umgestellt sind.                                  */
const ROLLE_TL = "TL", ROLLE_KB = "KB", ROLLE_ADMIN = "Admin";
const istTLSatz = u => !!u && (u.rolle === "TL" || u.rolle === "Teamleader" ||
                               String(u.dk||"").startsWith("TL-DK"));
const istAdminSatz = u => !!u && (u.rolle === "Admin" || String(u.dk||"").startsWith("V-DK"));
const istKBSatz = u => !!u && !istTLSatz(u) && !istAdminSatz(u);
...
const rolleAus = dk => String(dk).startsWith("V-DK") ? ROLLE_ADMIN : ROLLE_KB;
const SEED_USERS = {
